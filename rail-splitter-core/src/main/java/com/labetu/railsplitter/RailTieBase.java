...
/**
 * A {@link RailTieBase} is the slf4j decorator, this class wraps current log calls and stores them for
 * later usage.
 */
abstract class RailTieBase implements RailTie {
  ...
  /**
   * Creates a new {@link RailTieBase} with a stored logger.
   *
   * @param logger The logger to write messages with.
   * @param configuration The configuration to use.
   */
  RailTieBase(final Logger logger, final Switchman configuration) {
  ...
  public RailStats getStatistics() {
    // Statistics tracking is not enabled by default as it would be costly to run at all times.
    return null;
  }
  ...
  private void flushQueue() {
    ...
    if (configuration.logFutureMessages()) {
